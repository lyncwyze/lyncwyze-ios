...
struct PhoneVerificationView: View {
    ...
    var body: some View {
                VStack(spacing: 0) {
                    VStack(spacing: 16) {
                        ...
                        Text("sent on your phone number \(mobileNumber)")
                            .font(.system(size: 16))
                    ...
                    Button(
                        action: {
                            Task {
                                do {
                                    isLoading = true
                                    let success = try await NetworkManager.shared.resendPhoneOtpAsync(phoneNumber: mobileNumber)
                                    isLoading = false
                                    isSuccess = success
    ...
    private func verifyOTP() async {
        ...
        NetworkManager.shared.makeRequest(
            endpoint: "/user/verifyOtpAndCreateUser/\(otp)/\(mobileNumber)",
            method: .GET,
            headers: ["token": fcmToken,
                      "type": "mobile",
                      "platform": "iOS",
            ...
            DispatchQueue.main.async {
                ...
                case .success(let response):
                    ...
                    self.isSuccess = true
                    self.alertMessage = NSLocalizedString("otp_verified_successfully", comment: "")
                    self.showAlert = true
                ...
                case .failure(let error):
                    ...
                    if let networkError = error as? NetworkError {
                        ...
                        case .apiError(let message):
                            self.alertMessage = String(format: NSLocalizedString("verification_failed", comment: ""), message)
                        case .invalidURL:
                        ...
                        case .encodingError:
                            self.alertMessage = NSLocalizedString("error_processing_encode", comment: "")
                        }
                    } else {
...
struct PhoneVerificationView_Previews: PreviewProvider {
    static var previews: some View {
        PhoneVerificationView(mobileNumber: "+1987654210")
    }
}
